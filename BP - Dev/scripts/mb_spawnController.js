...
import {
    ...
    getClusterSpawnPressureMult,
    getClusterCampRamp01,
    getLastOverworldStormCampRamp01,
...
/** Same XZ threshold as tight-group / spawn cluster logic (blocks). */
const SPATIAL_CLUSTER_LINK_DISTANCE_SQ = 32 * 32;

/**
 * Union-find clusters by XZ proximity; returns cluster count and stable index 0..k-1 per player id.
 */
export function computeSpatialClusterMeta(players) {
    const empty = { count: 0, byPlayerId: new Map() };
    if (!players || players.length === 0) return empty;
    if (players.length === 1) {
        const p0 = players[0];
        if (!p0?.id) return empty;
        return { count: 1, byPlayerId: new Map([[p0.id, 0]]) };
    }
    ...
    for (let i = 0; i < n; i++) {
        const locI = players[i]?.location;
        if (!locI) continue;
        for (let j = i + 1; j < n; j++) {
            const locJ = players[j]?.location;
            if (!locJ) continue;
            const dx = locI.x - locJ.x;
            const dz = locI.z - locJ.z;
            if (dx * dx + dz * dz <= SPATIAL_CLUSTER_LINK_DISTANCE_SQ) union(i, j);
        }
    }
    ...
    for (let i = 0; i < n; i++) {
        ...
        byPlayerId.set(p.id, rootToIndex.get(r));
    }
    return { count: Math.max(1, rootToIndex.size), byPlayerId };
}

function countSpatialProximityClusters(players) {
    return computeSpatialClusterMeta(players).count;
}
...
/** Baseline scan tuning — aligned with journal Mid tier (`lowLag` preset). */
const DEFAULT_SCAN_SETTINGS = {
    discoveryRadius: 48,
    minDiscoveryRadius: 22,
    perPlayerRadiusDrop: 0.24,
    tightGroupRadiusPenalty: 0.38,
    barrenCooldownMult: 2.05,
    staggerTicks: 40,
    chunkLoadDelay: 12
};
...
export const SPAWN_SCAN_PRESETS = {
    ...
    /** Heavier throttling when many players or large worlds struggle. */
    minimal: {
        label: "Minimal Scan",
        discoveryRadius: 40,
        minDiscoveryRadius: 20,
        perPlayerRadiusDrop: 0.3,
        tightGroupRadiusPenalty: 0.45,
        barrenCooldownMult: 2.45,
        staggerTicks: 52,
        chunkLoadDelay: 16
    },
    /** Many players spread apart: smaller discovery, more stagger (pairs with runtime cluster budgeting). */
    multiplayerSpread: {
        label: "MP Spread (6–8p)",
        discoveryRadius: 44,
        minDiscoveryRadius: 21,
        perPlayerRadiusDrop: 0.26,
        tightGroupRadiusPenalty: 0.4,
        barrenCooldownMult: 2.15,
        staggerTicks: 46,
        chunkLoadDelay: 14
    },
    /** Softer scan shrink per extra player — good when usually solo on a server world. */
    soloHost: {
        label: "Solo / LAN host",
        discoveryRadius: 84,
        minDiscoveryRadius: 30,
        perPlayerRadiusDrop: 0.09,
        tightGroupRadiusPenalty: 0.15,
...
export const SPAWN_INTENSITY_PRESETS = {
    ...
    med: { label: "§fMed", desc: "Balanced (no journal overrides)", blockQuery: 1, maxGlobal: 24, range: "normal", tileIntensity: 1, blocksPerTick: 1, spawnSpeed: 1, spawnDifficulty: 0 },
...
/**
 * How many "logical" players to use for discovery-radius shrink when scanLoadCount is set
 * (spatial clusters in the dimension). Caps at real player count.
 */
function getDiscoveryBudgetPlayerCount(totalPlayerCount, scanLoadCount) {
    if (totalPlayerCount <= 1) return 1;
    const c = scanLoadCount != null && scanLoadCount >= 1 ? scanLoadCount : totalPlayerCount;
    return Math.max(1, Math.min(totalPlayerCount, c));
}

/**
 * Tier for block-query / per-scan tile limits: in MP, never below duo tier so one stacked group does not get solo-sized scans.
 */
...
function getAdaptiveDiscoveryRadius(totalPlayerCount, isTightGroup, isIsolated, isNewChunk = false, scanLoadCount = null) {
    const settings = getScanSettings();
    let radius = settings.discoveryRadius;
    const dc = getDiscoveryBudgetPlayerCount(totalPlayerCount, scanLoadCount);
    if (dc > 1) {
        const extraPlayers = dc - 1;
...
function getBarrenCooldownTicks(totalPlayerCount, chunkKey = "", scanLoadCount = null) {
    const base = SCAN_INTERVAL * 3;
    const settings = getScanSettings();
    let playerMult = 1;
    if (totalPlayerCount === 2) playerMult = 1.22;
    else if (totalPlayerCount === 3) playerMult = 1.42;
    else if (totalPlayerCount === 4) playerMult = 1.62;
    else if (totalPlayerCount === 5) playerMult = 1.82;
    else if (totalPlayerCount === 6) playerMult = 2.02;
    else if (totalPlayerCount === 7) playerMult = 2.18;
    else if (totalPlayerCount >= 8) playerMult = 2.35;
    // Many bodies in few clusters (stacked): lengthen barren wait so one empty chunk is not hammered.
    let stackedBoost = 0;
    if (scanLoadCount != null && totalPlayerCount > 1 && scanLoadCount >= 1 && totalPlayerCount > scanLoadCount) {
        stackedBoost = Math.min(0.55, (totalPlayerCount - scanLoadCount) * 0.07);
    }
    let hash = 0;
    ...
    return Math.floor(base * (playerMult + stackedBoost) * settings.barrenCooldownMult) + jitter;
}

/** One player processed per tick when spread out — interval grows with player count (up to 8). */
function getSpreadPlayerStaggerInterval(playerCount) {
    if (playerCount <= 1) return 1;
    if (playerCount === 2) return 4;
    if (playerCount === 3) return 7;
    if (playerCount === 4) return 11;
    if (playerCount === 5) return 15;
    if (playerCount === 6) return 19;
    if (playerCount === 7) return 23;
    return 28;
}
...
let cachedWorldSpawnLoadForCap = 1;
let cachedWorldSpawnLoadForCapTick = -1;
...
function getWorldWideSpawnLoadCount() {
    try {
        ...
        for (const p of pl) {
            ...
            if (!byDim.has(id)) byDim.set(id, []);
            byDim.get(id).push(p);
        }
        let sum = 0;
        for (const list of byDim.values()) {
            if (isSpatialSpawnTuningEnabled()) {
                sum += Math.max(1, computeSpatialClusterMeta(list).count);
            } else {
                sum += Math.max(1, list.length);
            }
        }
...
/** Same as property cap, scaled by world-wide spatial cluster load (cached per tick). */
function getEffectiveMaxGlobalSpawnsPerTick() {
    const base = getMaxGlobalSpawnsPerTick();
    const now = system.currentTick;
    if (cachedWorldSpawnLoadForCapTick !== now) {
        cachedWorldSpawnLoadForCapTick = now;
        try {
            cachedWorldSpawnLoadForCap = getWorldWideSpawnLoadCount();
        } catch {
            cachedWorldSpawnLoadForCap = 1;
        }
    }
...
/** Effective max candidates per scan (tile intensity + mild boost when block-query budget is compressed). */
function getEffectiveMaxCandidates(totalPlayerCount = 1, isSinglePlayer = false, scanLoadCount = null) {
...
/** Floor for block-query budget after per-player mobility throttle (never zero out scans). */
const MOBILITY_BLOCK_QUERY_FLOOR = 28;
function clampMobilityQueryMult(m) {
    if (m == null || typeof m !== "number" || !Number.isFinite(m) || m <= 0) return 1;
    return Math.min(1, Math.max(0.55, m));
}
...
function getBlockQueryLimit(isSinglePlayer, totalPlayerCount, scanLoadCount = null) {
    ...
    const qc = getQueryBudgetPlayerCount(totalPlayerCount, scanLoadCount);
    if (qc > 1) {
        const multiplier = qc === 2 ? 0.5 : qc === 3 ? 0.35 : 0.2;
...
const SCAN_YIELD_BALANCE_CAP = 2.25;

/**
 * Reference block-query budget if every player in the dimension counted separately for MP tier scaling (no dev mult).
 * Used to detect when actual budget is compressed and should be balanced with higher per-tick work / spawns.
 */
function getReferenceBlockQueryLimitForYieldBalance(isSinglePlayer, totalPlayerCount) {
    let baseLimit = isSinglePlayer ? MAX_BLOCK_QUERIES_SINGLE_PLAYER : MAX_BLOCK_QUERIES_PER_SCAN;
    const qc = Math.max(1, totalPlayerCount);
    if (qc > 1) {
        const multiplier = qc === 2 ? 0.5 : qc === 3 ? 0.35 : 0.2;
        baseLimit = Math.floor(baseLimit * multiplier);
    }
    return Math.max(500, baseLimit);
}

/**
 * When actual query limit is below the spread-MP reference (e.g. block query mult &lt; 1), return &gt;1 to bump
 * blocks-per-tick, candidate caps, and per-tick spawn limits. Stacked clusters already have a high limit → stays 1.
 */
function getScanYieldBalanceMultiplier(isSinglePlayer, totalPlayerCount, scanLoadCount = null) {
    const refLim = getReferenceBlockQueryLimitForYieldBalance(isSinglePlayer, totalPlayerCount);
    const actualLim = getBlockQueryLimit(isSinglePlayer, totalPlayerCount, scanLoadCount);
    if (actualLim >= refLim) return 1;
    const raw = refLim / actualLim;
    return Math.min(SCAN_YIELD_BALANCE_CAP, Math.max(1, raw));
}
...
function getSpawnDifficultyState() {
    ...
    if (typeof rawValue !== "number") {
        rawValue = 0;
        try {
            setWorldProperty(SPAWN_DIFFICULTY_PROPERTY, rawValue);
        } catch { /* ignored */ }
    }
    ...
    if (clampedValue !== rawValue) {
        try {
            setWorldProperty(SPAWN_DIFFICULTY_PROPERTY, clampedValue);
        } catch { /* ignored */ }
    }
...
// Isolated players get 50% longer TTL to reduce queries further
function getEntityCountCacheTTL(totalPlayerCount, isIsolated = false) {
    let baseTTL;
    if (totalPlayerCount === 1) baseTTL = SCAN_INTERVAL * 4; // 12 seconds for single player
    else if (totalPlayerCount === 2) baseTTL = SCAN_INTERVAL * 6; // 18 seconds for 2 players
    else baseTTL = SCAN_INTERVAL * 8; // 24 seconds for 3+ players
    
    // Isolated players get 50% longer TTL (reduces queries)
    if (isIsolated) {
...
/** Players who ran a tile rescan this spawn tick — defer heavy getEntities / buff ambience same tick. */
const playersTriggeredTileRescan = new Set();
...
const PROGRESSIVE_SCAN_TIMEOUT = SCAN_INTERVAL * 4; // Resume partial block scans within ~12s
const BLOCKS_PER_TICK_BUDGET = {
    1: 1800,  // Single player
    2: 1100,  // 2 players
    3: 750,   // 3 players
    4: 420    // 4+ players
};
/** Rotate which group member is tile-scanned each group rescan (spread MP load). */
const groupScanPlayerRotate = new Map(); // groupId -> next player index
...
function isPlayerIsolated(player, allPlayers) {
    ...
    if (cached && cached.tick === currentTick) {
        return { isolated: cached.isolated, nearbyPlayerCount: cached.nearbyPlayerCount };
    }
    ...
    // Clean old cache entries (older than 1 tick)
    if (isolatedPlayerCache.size > allPlayers.length * 2) {
        for (const [id, data] of isolatedPlayerCache.entries()) {
            if (currentTick - data.tick > 1) {
                isolatedPlayerCache.delete(id);
            }
        }
    }
    
    return { isolated, nearbyPlayerCount };
}
...
function getMaxSpawnsPerTick(day, totalPlayerCount = 1, scanYieldBalanceMult = 1) {
    ...
    if (totalPlayerCount > 1) {
        ...
        else if (totalPlayerCount === 4) multiplier = 0.34;
        else if (totalPlayerCount === 5) multiplier = 0.28;
        else if (totalPlayerCount === 6) multiplier = 0.24;
    ...
    const yb = (scanYieldBalanceMult != null && Number.isFinite(scanYieldBalanceMult) && scanYieldBalanceMult > 1)
        ? scanYieldBalanceMult
        : 1;
    if (yb > 1) {
        baseLimit = Math.max(1, Math.floor(baseLimit * yb));
        const hardCap = Math.ceil(MAX_SPAWNS_PER_TICK_PER_PLAYER_MAX * 1.35);
        baseLimit = Math.min(hardCap, baseLimit);
    }
...
/**
 * Desired total Maple Bear count near a player (same radius as entity count cache) for pacing.
 * Scales with day and dimension player count; not a hard cap — type caps and per-config maxCount still apply.
 */
function getIdealNearbyBearTarget(day, dimensionPlayerCount, dimensionId) {
    const pc = Math.max(1, dimensionPlayerCount);
    let base;
    if (day < 2) base = 7;
    else if (day < 4) base = 13;
    else if (day < 8) base = 22;
    else if (day < 13) base = 34;
    else if (day < 20) base = 46;
    else base = 56 + Math.min(30, Math.floor((day - 20) / 2));

    let dimScale = 1;
    if (dimensionId === "minecraft:the_end") dimScale = 0.78;
    else if (dimensionId === "minecraft:nether") dimScale = 0.88;

    const mpScale = pc === 1 ? 1 : pc === 2 ? 1.08 : pc === 3 ? 1.15 : 1.22;
    return Math.max(10, Math.floor(base * mpScale * dimScale));
}

/**
 * Compare actual nearby bear total to ideal: below ideal → easier spawns; near ideal → neutral; above → softer.
 */
function getIdealBearPressureFactors(nearbyTotal, ideal) {
    const target = Math.max(10, ideal);
    const n = Math.max(0, nearbyTotal);
    ...
    return { chanceMult, spawnRateMult, ratio, target };
}
...
function tryAcquireChunkFullScanSlot() {
    const now = system.currentTick;
    if (now !== chunkFullScansStartedTick) {
        chunkFullScansStartedTick = now;
        chunkFullScansStartedCount = 0;
    }
    if (chunkFullScansStartedCount >= MAX_CHUNK_FULL_SCANS_PER_TICK) return false;
    chunkFullScansStartedCount++;
    return true;
}
...
const CACHE_VALIDATION_INTERVAL = SCAN_INTERVAL * 3; // Start a new validation wave this often
const CACHE_VALIDATION_SAMPLE_SIZE = 50; // Max blocks per wave (spread across many ticks)
const CACHE_VALIDATION_BLOCKS_PER_TICK = 10; // getBlock calls per collectDustedTiles invocation (reduces spikes)
let cacheValidationQueue = null; // { pairs: [entries], idx: number }
let cacheValidationWaveStartTick = -999999;
...
// Validate cached blocks in small batches (avoids 50× getBlock in one tick).
function validateDustedDirtCache(dimension) {
    const now = system.currentTick;

    if (!cacheValidationQueue) {
        if (now - cacheValidationWaveStartTick < CACHE_VALIDATION_INTERVAL) {
            return;
        }
        const entries = Array.from(dustedDirtCache.entries())
            ...
            .sort((a, b) => a[1].tick - b[1].tick);
        const sampleSize = Math.min(CACHE_VALIDATION_SAMPLE_SIZE, entries.length);
        if (sampleSize === 0) {
            return;
        }
        cacheValidationWaveStartTick = now;
        cacheValidationQueue = {
            pairs: entries.slice(0, sampleSize),
            idx: 0
        };
    }

    const q = cacheValidationQueue;
    const toRemove = [];
    const end = Math.min(q.idx + CACHE_VALIDATION_BLOCKS_PER_TICK, q.pairs.length);
    while (q.idx < end) {
        const [key, value] = q.pairs[q.idx];
        q.idx++;

        try {
            // Always resolve block dimension from cache entry so Nether/OW can share draining one queue without skipping work.
            const checkDim = (value.dimension ? world.getDimension(value.dimension) : null) || dimension || null;
            if (checkDim) {
...
function collectMiningSpawnTiles(dimension, center, minDistance, maxDistance, limit = MAX_CANDIDATES_PER_SCAN, isSinglePlayer = false, totalPlayerCount = 1, isTightGroup = false, isIsolated = false, scanLoadCount = null, spatialClusterIndex = 0, chunkFairnessClusterCount = 1, mobilityQueryMult = 1, scanSessionId = "") {
    ...
    if (queryPc > 1) {
        ...
        const multiplier = queryPc === 2 ? 0.9 : queryPc === 3 ? 0.8 : 0.7;
    ...
    const mm = clampMobilityQueryMult(mobilityQueryMult);
    let queryLimit = getBlockQueryLimit(isSinglePlayer, totalPlayerCount, scanLoadCount);
    queryLimit = Math.max(MOBILITY_BLOCK_QUERY_FLOOR, Math.floor(queryLimit * mm));
    ...
    if (isDebugEnabled('spawn', 'tileScanning') || isDebugEnabled('spawn', 'all')) {
        ...
        const adaptiveDiscovery = getAdaptiveDiscoveryRadius(totalPlayerCount, isTightGroup, isIsolated, false, scanLoadCount);
    ...
    const dustedTiles = collectDustedTiles(dimension, center, minDistance, maxDistance, Math.floor(limit * 0.6), isSinglePlayer, totalPlayerCount, isTightGroup, isIsolated, scanLoadCount, spatialClusterIndex, chunkFairnessClusterCount, mobilityQueryMult, scanSessionId); // 60% from dusted_dirt
...
function collectDustedTiles(dimension, center, minDistance, maxDistance, limit = MAX_CANDIDATES_PER_SCAN, isSinglePlayer = false, totalPlayerCount = 1, isTightGroup = false, isIsolated = false, scanLoadCount = null, spatialClusterIndex = 0, chunkFairnessClusterCount = 1, mobilityQueryMult = 1, scanSessionId = "") {
    ...
    const queryPc = getQueryBudgetPlayerCount(totalPlayerCount, scanLoadCount);
    // Reduce candidate limit when multiple players are present (less tiles needed per player)
    // Isolated players get 40% reduction in tile scan limit
    let effectiveLimit = limit;
    if (isIsolated) {
        effectiveLimit = Math.floor(limit * 0.6); // 40% reduction for isolated players
    } else if (queryPc > 1) {
        if (isTightGroup) {
    ...
    let baseQueryLimit = getBlockQueryLimit(isSinglePlayer, totalPlayerCount, scanLoadCount);
    if (isTightGroup) {
    ...
    if (cachedTiles.length >= MIN_CACHED_TILES_FOR_FULL_SCAN && blockQueryCount < queryLimit) {
        ...
        const minTilesNeeded = discoveryPc >= 3 ? Math.floor(limit * 0.2) : Math.floor(limit * 0.3); // 20% for 3+ players, 30% for 1-2 players
        const minCachedForSkip = discoveryPc >= 4 ? 3 : discoveryPc >= 3 ? 4 : MIN_CACHED_TILES_FOR_FULL_SCAN; // Lower threshold for 3+ players, even lower for 4+
    ...
    if (!isNetherOrEnd) {
        ...
        const barrenCooldownTicks = getBarrenCooldownTicks(totalPlayerCount, chunkKey, scanLoadCount);
    ...
    const fairnessRotate = Math.max(0, Math.floor(now / Math.max(4, getScanStaggerTicks())));
    const readyScans = Array.from(chunkScanQueue.entries())
        ...
        .sort((a, b) => {
            ...
            const rb = (cb + fairnessRotate) % 997;
            return ra - rb;
        });
    ...
    let discoveryRadius = getAdaptiveDiscoveryRadius(totalPlayerCount, isTightGroup, isIsolated, isNewChunk, scanLoadCount);
    if (isNewChunk) {
    ...
    // Per-tick block budget: tier by effective scan load (clusters), not raw headcount; boost when query budget is compressed
    const qcBudget = Math.max(1, getQueryBudgetPlayerCount(totalPlayerCount, scanLoadCount));
    const baseBudget = BLOCKS_PER_TICK_BUDGET[Math.min(qcBudget, 4)] || BLOCKS_PER_TICK_BUDGET[4];
    const yieldBal = getScanYieldBalanceMultiplier(isSinglePlayer, totalPlayerCount, scanLoadCount);
    let blocksPerTickBudget = Math.max(160, Math.floor(baseBudget * getBlocksPerTickMultiplier() * yieldBal));
    const queueDepth = chunkScanQueue.size;
    if (queueDepth > CHUNK_SCAN_QUEUE_DEFER_DEPTH) {
        blocksPerTickBudget = Math.floor(blocksPerTickBudget * 0.55);
    } else if (queueDepth > 4) {
        blocksPerTickBudget = Math.floor(blocksPerTickBudget * 0.72);
    }
    ...
    const partialKey = scanSessionId ? `${scanSessionId}` : "";
    const scanNow = system.currentTick;
    let xzResumeIndex = 0;
    if (partialKey) {
        const partial = progressiveBlockScanCache.get(partialKey);
        if (partial && (scanNow - (partial.lastTick ?? 0)) <= PROGRESSIVE_SCAN_TIMEOUT && partial.xzIndex < xzPositionsByDistance.length) {
            xzResumeIndex = Math.max(0, partial.xzIndex | 0);
            if (isDebugEnabled('spawn', 'tileScanning') || isDebugEnabled('spawn', 'all')) {
                console.warn(`[SPAWN DEBUG] Resuming partial scan ${partialKey} at XZ ${xzResumeIndex}/${xzPositionsByDistance.length}`);
            }
        } else if (partial) {
            progressiveBlockScanCache.delete(partialKey);
        }
    }
    
    // Scan XZ positions (center-out order), checking Y levels — may pause and resume via progressiveBlockScanCache
    let xzLoopIndex = xzResumeIndex;
    for (; xzLoopIndex < xzPositionsByDistance.length; xzLoopIndex++) {
        const pos = xzPositionsByDistance[xzLoopIndex];
        const x = pos.x;
        const z = pos.z;
        // Early exit: If we have enough candidates, stop scanning (especially important for Nether/End)
        if (candidates.length >= limit) {
            if (partialKey) progressiveBlockScanCache.delete(partialKey);
            if (isDebugEnabled('spawn', 'tileScanning') || isDebugEnabled('spawn', 'all')) {
        ...
        if (blocksCheckedThisTick >= blocksPerTickBudget) {
            if (partialKey) {
                progressiveBlockScanCache.set(partialKey, { xzIndex: xzLoopIndex, lastTick: scanNow });
            }
            if (isDebugEnabled('spawn', 'tileScanning') || isDebugEnabled('spawn', 'all')) {
                console.warn(`[SPAWN DEBUG] Scan paused: Per-tick budget reached (${blocksCheckedThisTick}/${blocksPerTickBudget}), resume XZ ${xzLoopIndex}/${xzPositionsByDistance.length}`);
            }
            break;
        }
    ...
    if (partialKey && xzLoopIndex >= xzPositionsByDistance.length) {
        progressiveBlockScanCache.delete(partialKey);
    }
    ...
    if (!isNetherOrEnd && typeof candidatesBeforeScan !== 'undefined') {
        ...
        const minQueriesForBarrenMark = Math.max(48, Math.floor(queryLimit * 0.14));
        const scanHitBudgetCap = blockQueryCount >= queryLimit;
        if (newBlocksFound === 0 && blockQueryCount >= minQueriesForBarrenMark && !scanHitBudgetCap) {
            const chunkKey = getChunkKey(cx, cz);
            const posKey = `${cx},${cz}`;
...
function getTilesForPlayer(player, dimension, playerPos, currentDay, useGroupCache = false, groupPlayers = null, totalPlayerCount = 1, isTightGroupParam = false, isIsolated = false, scanLoadCount = null, spatialClusterIndex = 0, chunkFairnessClusterCount = 1, mobilityQueryMult = 1) {
    const playerId = player.id;
    const now = system.currentTick;
    ...
    if (needsRescan) {
        ...
        playersTriggeredTileRescan.add(playerId);
        if (useGroupCache && groupPlayers && groupPlayers.length > 1) {
            for (const gp of groupPlayers) {
                if (gp?.id) playersTriggeredTileRescan.add(gp.id);
            }
        }
            ...
            if (group && group.players.length > 1) {
                ...
                if (!scannedGroupsThisTick.has(groupId)) {
                    ...
                    if (boundingRect) {
                        ...
                        const gClusterMeta = computeSpatialClusterMeta(group.players);
                        const gFairN = Math.max(1, gClusterMeta.count);
                        const existingGroupCache = groupCaches.get(groupId);
                        if (existingGroupCache?.tiles?.length) {
                            for (const tile of existingGroupCache.tiles) {
                                const key = `${tile.x},${tile.y},${tile.z}`;
                                if (!allTiles.has(key)) allTiles.set(key, tile);
                            }
                        }
                        let rotIdx = groupScanPlayerRotate.get(groupId) ?? 0;
                        const p = group.players[rotIdx % group.players.length];
                        groupScanPlayerRotate.set(groupId, (rotIdx + 1) % group.players.length);
                        debugLog('groups', `Scanning group ${groupId} — one member this tick: ${p.name} (${rotIdx + 1}/${group.players.length})`);
                        const pPos = getActualPlayerPosition(p) || p.location;
                        const pci = gClusterMeta.byPlayerId.get(p.id) ?? 0;
                        const pMob = getPlayerMobilityQueryMult(p.id);
                        const playerTiles = collectMiningSpawnTiles(dimension, pPos, getMinSpawnDistance(), getMaxSpawnDistance(), getEffectiveMaxCandidates(totalPlayerCount, false, scanLoadCount), false, totalPlayerCount, isTightGroupParam, isIsolated, scanLoadCount, pci, gFairN, pMob, `${p.id},${dimension.id}`);
        ...
        if (!cache || !isGroupCache) {
            ...
            const tiles = collectMiningSpawnTiles(dimension, playerPos, getMinSpawnDistance(), getMaxSpawnDistance(), getEffectiveMaxCandidates(totalPlayerCount, isSinglePlayer, scanLoadCount), isSinglePlayer, totalPlayerCount, isTightGroupParam, isIsolated, scanLoadCount, spatialClusterIndex, chunkFairnessClusterCount, mobilityQueryMult, `${playerId},${dimension.id}`);
    ...
    const sampled = sampleTiles(validTiles, getEffectiveMaxCandidates(totalPlayerCount, isSinglePlayer, scanLoadCount));
    const spacedTiles = filterTilesWithSpacing(sampled, spacing, getEffectiveMaxSpacedTiles(totalPlayerCount, isSinglePlayer, scanLoadCount));
...
const MB_SPAWN_COUNT_PREFIXES = ["mb:mb_day00", "mb:infected", "mb:buff_mb", "mb:flying_mb", "mb:mining_mb", "mb:torpedo_mb"];

function isMbSpawnCountableType(typeId) {
    if (!typeId) return false;
    return MB_SPAWN_COUNT_PREFIXES.some((prefix) => typeId.startsWith(prefix));
}

/** Bear counts near a point via shared snapshot (no getEntities of all mob types). */
function countMbBearsNearPosition(dimension, centerPos, maxRadius) {
    const counts = {};
    if (!dimension || !centerPos) return counts;
    ...
    try {
        ...
        for (const entity of snap.all) {
            ...
            if (!isMbSpawnCountableType(typeId)) continue;
            try {
...
function getBatchEntityCounts(dimension, players, totalPlayerCount = 1) {
    ...
    // Per-player bear snapshot counts (avoids enumerating villagers/items in villages).
    try {
        ...
        for (const player of playersNeedingRefresh) {
            const actualPos = getActualPlayerPosition(player);
            if (!actualPos) continue;
            positions.push({ player, pos: actualPos });
        }
        ...
        for (const { pos } of positions) {
            ...
            maxPlayerDistance = Math.max(maxPlayerDistance, Math.sqrt(dx * dx + dz * dz));
        }

        const BATCH_ENTITY_MAX_PLAYER_DISTANCE = 80;
        if (maxPlayerDistance > BATCH_ENTITY_MAX_PLAYER_DISTANCE) {
            return results;
        }

        for (const { player, pos } of positions) {
            const counts = countMbBearsNearPosition(dimension, pos, maxRadius);
            entityCountCache.set(player.id, { counts, tick: now });
            results.set(player.id, counts);
        }
    } catch (error) {
...
function getEntityCountsForPlayer(player, dimension, playerPos, totalPlayerCount = 1, isIsolated = false) {
    const playerId = player.id;
    const now = system.currentTick;
    let cache = entityCountCache.get(playerId);
    
    const cacheTTL = getEntityCountCacheTTL(totalPlayerCount, isIsolated);
    const canDeferEntityQuery = playersTriggeredTileRescan.has(playerId) && cache && typeof cache.counts === "object" && now - cache.tick <= cacheTTL + SCAN_INTERVAL * 5;
    if ((!cache || now - cache.tick > cacheTTL) && !canDeferEntityQuery) {
        try {
            ...
            const counts = countMbBearsNearPosition(dimension, playerPos, maxRadius);
...
function attemptSpawnType(player, dimension, playerPos, tiles, config, modifiers = {}, entityCounts = {}, spawnCount = {}, nearbyPlayerCount = 1, totalPlayerCountInDimension = 1, isIsolated = false) {
    ...
    const yieldBalSpawn = (modifiers.scanYieldBalanceMult != null && Number.isFinite(modifiers.scanYieldBalanceMult) && modifiers.scanYieldBalanceMult > 0)
        ? modifiers.scanYieldBalanceMult
        : 1;
    const baseMaxSpawnsTick = getMaxSpawnsPerTick(currentDay, totalPlayerCountInDimension, yieldBalSpawn);
    const idealSpawnPress = (modifiers.idealBearSpawnRateMult != null && Number.isFinite(modifiers.idealBearSpawnRateMult) && modifiers.idealBearSpawnRateMult > 0)
        ? modifiers.idealBearSpawnRateMult
        : 1;
    const maxSpawnsPerTick = Math.max(1, Math.min(
        Math.ceil(MAX_SPAWNS_PER_TICK_PER_PLAYER_MAX * 1.35),
        Math.floor(baseMaxSpawnsTick * idealSpawnPress)
    ));
    ...
    if (globalSpawnCount >= getEffectiveMaxGlobalSpawnsPerTick()) {
    ...
    // Isolated players get 25% more base attempts to compensate for reduced scanning
    let baseAttempts = Math.max(1, SPAWN_ATTEMPTS + (modifiers.attemptBonus ?? 0));
    if (isIsolated) {
        baseAttempts = Math.floor(baseAttempts * 1.25); // 25% increase for isolated players
    }
    ...
    if (yieldBalSpawn > 1) {
        maxAttemptsPerTick = Math.max(1, Math.floor(maxAttemptsPerTick * Math.min(1.5, Math.sqrt(yieldBalSpawn))));
    }
    if (idealSpawnPress !== 1) {
    ...
    // Isolated players get 40% spawn chance multiplier to compensate for fewer tiles found
    if (isIsolated) {
        chance *= 1.4; // 40% increase for isolated players
    }
    ...
    // Isolated players get enhanced compensation (1.5x instead of 1.25x when tiles < 30)
    if (availableTiles < 10) {
        // Low tile density: Increase spawn chance by 20-30% (or 50% for isolated players)
        chance *= isIsolated ? 1.5 : 1.25;
    } else if (availableTiles < 30 && isIsolated) {
        // Isolated players with moderate tile density: Boost from 1.25x to 1.5x
        chance *= 1.5;
    ...
    const idealChancePress = (modifiers.idealBearPressureChanceMult != null && Number.isFinite(modifiers.idealBearPressureChanceMult) && modifiers.idealBearPressureChanceMult > 0)
        ? modifiers.idealBearPressureChanceMult
        : 1;
    chance *= idealChancePress;
    ...
    for (let i = 0; i < finalAttemptsThisTick && spawnCount.value < maxSpawnsPerTick && globalSpawnCount < getEffectiveMaxGlobalSpawnsPerTick(); i++) {
...
/** Centroid of players in one spatial cluster (same index as `computeSpatialClusterMeta`). */
function computeClusterCentroid(dimPlayers, meta, clusterIndex) {
    let sx = 0;
    let sy = 0;
    let sz = 0;
    let n = 0;
    for (const pl of dimPlayers) {
        if (!pl?.isValid || !pl.location) continue;
        if ((meta.byPlayerId.get(pl.id) ?? 0) !== clusterIndex) continue;
        sx += pl.location.x;
        sy += pl.location.y;
        sz += pl.location.z;
        n++;
...
/** Wall-time-ish ETA from remaining sedentary ticks (20 t/s). */
function formatCampTimeToMaxHud(remainingTicks) {
    const sec = Math.max(0, Math.ceil(remainingTicks / 20));
    if (sec <= 0) return "§amax§r";
    if (sec >= 3600) return `§e~${Math.round(sec / 3600)}h§r`;
    if (sec >= 60) return `§e~${Math.ceil(sec / 60)}m§r`;
    return `§e~${sec}s§r`;
}
...
system.runInterval(() => {
    try {
        ...
        for (const p of players) {
            ...
            const allowed = (p.hasTag("mb_cheats") || p.name === "Litbolt123");
            if (!p.hasTag?.("mb_dev_camp_watch") || !allowed) {
                if (!allowed && p.hasTag?.("mb_dev_camp_watch")) {
                    try { p.removeTag("mb_dev_camp_watch"); } catch { /* ignore */ }
                }
                clearHudActionBarSegment(p, ACTION_BAR_SLOT.CAMP_DEV);
                continue;
            }
            const dim = p.dimension;
            if (!dim?.id) continue;
            const dimPlayers = dim.getPlayers();
            const meta = computeSpatialClusterMeta(dimPlayers);
            const ci = meta.byPlayerId.get(p.id) ?? 0;
            const ramp = getClusterCampRamp01(dim.id, ci);
            const mult = getClusterSpawnPressureMult(dim.id, ci);
            const mobQ = getPlayerMobilityQueryMult(p.id);
            const owStormR = getLastOverworldStormCampRamp01();
            const stormSc = getStormStartChanceCampScale();
            const centroid = computeClusterCentroid(dimPlayers, meta, ci);
            const campDbg = centroid
                ? getClusterCampDebugMetrics(dim.id, ci, centroid.x, centroid.y, centroid.z)
                : null;
            const fullRampTicks = campDbg?.rampFullTicks ?? CAMP_RAMP_FULL_TICKS;
            let toMaxToken;
            if (ramp >= 0.999) {
                toMaxToken = "§amax§r";
            } else if (!campDbg?.hasState) {
                toMaxToken = `§7+${formatCampTimeToMaxHud(CAMP_RAMP_FULL_TICKS)}§r`;
            } else {
                const rem = Math.max(0, fullRampTicks - campDbg.sedentaryTicks);
                const eta = formatCampTimeToMaxHud(rem);
...
system.runInterval(() => {
    try {
        ...
        if (isSpreadThrottleActive()) {
            if (claimSpreadSlice("spawnHud", 80)) {
...
try {
    system.runInterval(() => {
        if (getCurrentDay() < 2) return;
        if (!claimSpreadSlice("spawnPresetAuto", 100)) return;
...
system.runInterval(() => {
    try {
        ...
        globalSpawnCount = 0;
        playersTriggeredTileRescan.clear();

        // Cleanup old cache entries periodically
        if (system.currentTick % (SCAN_INTERVAL * 5) === 0) {
            try {
                const partialStaleTick = system.currentTick - PROGRESSIVE_SCAN_TIMEOUT * 2;
                for (const [key, partial] of progressiveBlockScanCache.entries()) {
                    if ((partial?.lastTick ?? 0) < partialStaleTick) progressiveBlockScanCache.delete(key);
                }
    ...
    // Spawn scan perf HUD: refreshed every 10t in its own interval (not here — throttled / day gate).
    ...
    for (const dimId of dimensions) {
        const plist = playersByDimension.get(dimId);
        if (!plist?.length) continue;
        try {
            const meta = computeSpatialClusterMeta(plist);
            tickMobilityCampForDimension(dimId, plist, meta, nowTick);
        } catch (error) {
            errorLog("tickMobilityCampForDimension", error, { dimension: dimId });
        }
    }
    ...
    let dimClusterMeta;
    let dimensionSpatialClusterCount = dimensionPlayerCount;
    try {
        dimClusterMeta = computeSpatialClusterMeta(dimensionPlayers);
        dimensionSpatialClusterCount = Math.max(1, dimClusterMeta.count);
    } catch {
        dimClusterMeta = { count: dimensionPlayerCount, byPlayerId: new Map() };
        dimensionSpatialClusterCount = dimensionPlayerCount;
    }
    if (!isSpatialSpawnTuningEnabled()) {
        dimensionSpatialClusterCount = Math.max(1, dimensionPlayerCount);
    }
    ...
    // Process selected players (spread mode adds at most one; tight group adds one representative)
    const MAX_SPAWN_PLAYERS_PER_CONTROLLER_TICK = 1;
    let spawnPlayersHandled = 0;
    for (const player of playersToProcess) {
        if (!player) continue;
        if (spawnPlayersHandled >= MAX_SPAWN_PLAYERS_PER_CONTROLLER_TICK) break;
        spawnPlayersHandled++;
        ...
        // Detect if player is isolated (>96 blocks from all other players)
        const isolationInfo = isPlayerIsolated(player, allPlayers);
        const isIsolated = isolationInfo.isolated;
        ...
        let tileInfo;
        try {
            const playerClusterIndex = dimClusterMeta.byPlayerId.get(player.id) ?? 0;
            const mobilityQueryMult = getPlayerMobilityQueryMult(player.id);
            tileInfo = getTilesForPlayer(player, dimension, playerPos, currentDay, useGroupCache, dimensionPlayers, dimensionPlayerCount, isTightGroupMode, isIsolated, dimensionSpatialClusterCount, playerClusterIndex, dimensionSpatialClusterCount, mobilityQueryMult);
        } catch (error) {
        ...
        try {
            if (batchEntityCounts.has(player.id)) {
                ...
                entityCounts = getEntityCountsForPlayer(player, dimension, playerPos, dimensionPlayerCount, isIsolated);
            }
        } catch (error) {
            errorLog(`Error getting entity counts for player ${player.name}`, error, {
                player: player.name,
        ...
        debugLog('spawn', `${player.name}: ideal neighborhood ~${idealNearbyBearTarget} bears (${(idealBearPressure.ratio * 100).toFixed(0)}% of target) → spawn pressure chance×${idealBearPressure.chanceMult.toFixed(2)} rate×${idealBearPressure.spawnRateMult.toFixed(2)}`);
        ...
        for (const config of SPAWN_CONFIGS) {
            ...
            try {
                ...
                const spawned = attemptSpawnType(player, dimension, playerPos, spacedTiles, config, configModifiers, entityCounts, spawnCount, nearbyPlayerCount, dimensionPlayerCount, isIsolated);
