...
function tryRefreshInfectionHudActionBar(player, id, state) {
    ...
    try {
        ...
        if (cureHint && timerLine) {
            infectionLine = `${cureHint} §8| ${timerLine}`;
        } else if (cureHint) {
            infectionLine = cureHint;
        } else if (timerLine) {
    ...
    switch (lower) {
        ...
        case "summon_storm": {
            ...
            if (!isDustStormsEnabled()) {
                sender.sendMessage(CHAT_DEV + "[MBI] " + CHAT_INFO + "Turn on dust storms in Developer Tools → Storm hub (dust storms world toggle).");
                return;
            }
