...
import {
    ...
    devForceClusterAnchorPosition,
    devAddClusterSedentaryTicks,
    devSetClusterSedentaryTicks,
    devClearClusterCampState,
...
/** Spawn + scan rows for journal "Have lag?" tiers (aligned with SPAWN_PRESETS medLow / low / ultraLow). */
const JOURNAL_LAG_SPAWN_ROWS = {
    1: { blockQuery: 0.5, maxGlobal: 18, range: "close", tileIntensity: 0.75, blocksPerTick: 0.7, spawnSpeed: 0.75, spawnDifficulty: -1 },
    2: { blockQuery: 0.25, maxGlobal: 12, range: "close", tileIntensity: 0.5, blocksPerTick: 0.5, spawnSpeed: 0.5, spawnDifficulty: -1 },
    3: { blockQuery: 0.2, maxGlobal: 10, range: "close", tileIntensity: 0.45, blocksPerTick: 0.45, spawnSpeed: 0.35, spawnDifficulty: -1 }
};
const JOURNAL_LAG_SCAN_KEYS = { 1: "multiplayerSpread", 2: "lowLag", 3: "minimal" };
...
export function applyJournalLagComfortBundle(level) {
    ...
    const row = JOURNAL_LAG_SPAWN_ROWS[level];
    const scanKey = JOURNAL_LAG_SCAN_KEYS[level];
    if (row) {
        setWorldProperty(SPAWN_OVERRIDE_PROPERTIES.blockQueryMult, row.blockQuery);
        setWorldProperty(SPAWN_OVERRIDE_PROPERTIES.maxGlobal, row.maxGlobal);
        setWorldProperty(SPAWN_OVERRIDE_PROPERTIES.range, row.range);
        setWorldProperty(SPAWN_OVERRIDE_PROPERTIES.tileIntensity, row.tileIntensity);
        setWorldProperty(SPAWN_OVERRIDE_PROPERTIES.blocksPerTickMult, row.blocksPerTick);
        setWorldProperty(SPAWN_SPEED_PROPERTY, row.spawnSpeed);
        setWorldProperty(SPAWN_DIFFICULTY_PROPERTY, row.spawnDifficulty);
    }
    if (scanKey) applySpawnScanPreset(scanKey);
    try { saveAllProperties(); } catch { /* ignore */ }
}
...
export function openJournalLagComfortWizard(player, goBack) {
    ...
    form1.show(player).then((res) => {
        ...
        form2.show(player).then((res2) => {
            ...
            if (!res2 || res2.canceled || res2.selection === 4) {
...
export function isDevPreviewAdminMainMenuEnabled() {
    ...
    try {
        const v = getWorldProperty(MB_WORLD_DEV_PREVIEW_ADMIN_MAIN);
...
export function showCodexBook(player, context) {
    ...
    const DEVELOPER_TOOLS_DISCLAIMER_PROP = "mb_dev_tools_disclaimer_v1";
    const ADMIN_TOOLS_DISCLAIMER_PROP = "mb_admin_tools_disclaimer_v1";
    let journalPowerToolsBack = () => {};
    const forceSpawnNav = { rootBack: null, afterComplete: null };
    function runForceSpawnRootBack() {
        const fn = forceSpawnNav.rootBack;
        return (typeof fn === "function" ? fn : () => openSpawnControllerMenu())();
    }
    function runForceSpawnAfterComplete() {
        const fn = forceSpawnNav.afterComplete;
        return (typeof fn === "function" ? fn : () => openSpawnControllerMenu())();
    }
    ...
    function openListBearsMenu() {
        ...
        form.show(player).then((res) => {
            ...
            triggerDebugCommand("list_bears", args, () => journalPowerToolsBack());
        }).catch(() => journalPowerToolsBack());
    }
    ...
    function getMobilityCampDevContext(p) {
        try {
            ...
            for (const pl of dimPlayers) {
                if ((meta.byPlayerId.get(pl.id) ?? 0) !== ci) continue;
            ...
            const cx = n ? sx / n : p.location.x;
            const cy = n ? sy / n : p.location.y;
            const cz = n ? sz / n : p.location.z;
            return {
                dimId: dim.id,
                clusterIndex: ci,
                clusterCount: meta.count,
    ...
    function dumpMobilityCampFullDetailsToChat(p) {
        const ctx = getMobilityCampDevContext(p);
        if (!ctx) {
            p.sendMessage(CHAT_DANGER + "[Camp dev] No context.");
            return;
        }
        const tune = getCampTuningConstants();
        const m = getClusterCampDebugMetrics(ctx.dimId, ctx.clusterIndex, ctx.centroid.x, ctx.centroid.y, ctx.centroid.z);
        const mobQ = getPlayerMobilityQueryMult(p.id);
        const rampRead = getClusterCampRamp01(ctx.dimId, ctx.clusterIndex);
        const spawnM = getClusterSpawnPressureMult(ctx.dimId, ctx.clusterIndex);
        const owR = getLastOverworldStormCampRamp01();
        ...
        p.sendMessage(CHAT_INFO + `OW storm cache ramp01=${owR.toFixed(4)} stormStartRollScale=${stormSc.toFixed(4)}`);
        if (m.hasState) {
            p.sendMessage(CHAT_INFO + `Anchor: ${m.anchor.x.toFixed(2)}, ${m.anchor.y.toFixed(2)}, ${m.anchor.z.toFixed(2)}`);
            p.sendMessage(CHAT_INFO + `SedentaryTicks=${m.sedentaryTicks} lastTick=${m.lastTick}`);
            p.sendMessage(CHAT_INFO + `vs centroid: dXZ=${m.dxz.toFixed(2)} dY=${m.dy.toFixed(2)} insideCylinder=${m.sedentary ? "yes" : "no"}`);
        } else {
            p.sendMessage(CHAT_WARNING + `[Camp dev] No camp state for ${m.key} yet (spawn controller must tick this dimension).`);
        }
        const hud = getHudActionBarDebugInfo(p);
        if (hud.count > 0) {
            p.sendMessage(CHAT_INFO + `Merged action bar: ${hud.count} segment(s), slot priorities ${hud.slots.join(", ")}`);
            p.sendMessage(CHAT_INFO + `Preview: ${hud.preview.length > 220 ? hud.preview.slice(0, 220) + "…" : hud.preview}`);
        } else {
            p.sendMessage(CHAT_INFO + "Merged action bar: no active segments (infection / scan overlay / camp dev).");
        }
    }

    function openMobilityCampDevMenu() {
        ...
        form.show(player).then((res) => {
            ...
            player.playSound("mb.codex_turn_page", { pitch: 1.1, volume: 0.7 * v });
            if (!ctx) {
                player.sendMessage(CHAT_DANGER + "[Camp dev] No context.");
                return openMobilityCampDevMenu();
            }
            const tick = system.currentTick;
            const cx = ctx.centroid.x;
            const cy = ctx.centroid.y;
            const cz = ctx.centroid.z;
            const { dimId, clusterIndex } = ctx;
            if (res.selection === 0) {
                if (watchOn) {
                    try { player.removeTag("mb_dev_camp_watch"); } catch { /* ignore */ }
                    player.sendMessage(CHAT_INFO + "[Camp dev] Camp HUD off.");
                } else {
                    try { player.addTag("mb_dev_camp_watch"); } catch { /* ignore */ }
                    player.sendMessage(CHAT_INFO + "[Camp dev] Camp HUD on (action bar). Tag: mb_dev_camp_watch");
                }
            } else if (res.selection === 1) {
                dumpMobilityCampFullDetailsToChat(player);
            } else if (res.selection === 2) {
                devForceClusterAnchorPosition(dimId, clusterIndex, player.location.x, player.location.y, player.location.z, tick);
                player.sendMessage(CHAT_SUCCESS + "[Camp dev] Anchor snapped to your feet (sedentary ticks unchanged).");
            } else if (res.selection === 3) {
                const mSnap = getClusterCampDebugMetrics(dimId, clusterIndex, cx, cy, cz);
                const fullT = mSnap.hasState && mSnap.rampFullTicks ? mSnap.rampFullTicks : CAMP_RAMP_FULL_TICKS;
                const half = Math.floor(fullT / 2);
                devAddClusterSedentaryTicks(dimId, clusterIndex, half, tick, cx, cy, cz);
                player.sendMessage(CHAT_SUCCESS + `[Camp dev] +${half} sedentary ticks.`);
            } else if (res.selection === 4) {
                const mSnap = getClusterCampDebugMetrics(dimId, clusterIndex, cx, cy, cz);
                const fullT = mSnap.hasState && mSnap.rampFullTicks ? mSnap.rampFullTicks : CAMP_RAMP_FULL_TICKS;
                devSetClusterSedentaryTicks(dimId, clusterIndex, fullT, tick, cx, cy, cz);
                player.sendMessage(CHAT_SUCCESS + "[Camp dev] Sedentary = full ramp window.");
            } else if (res.selection === 5) {
                devClearClusterCampState(dimId, clusterIndex);
    ...
    /**
     * Release builds: one-time acknowledgement before any admin surface (menu or pinned storm/list).
     * Dev build: skipped when full developer tools are on.
     */
    function runAdminSurfaceWithDisclaimer(thenFn, opts) {
        if (typeof thenFn !== "function") return;
        const forcePublicDisclaimer = opts && opts.forcePublicDisclaimer === true;
        if (INCLUDE_FULL_DEVELOPER_TOOLS && !forcePublicDisclaimer) {
            thenFn();
            return;
        }
        try {
            const ack = getPlayerProperty(player, ADMIN_TOOLS_DISCLAIMER_PROP);
            if (ack === true || ack === "true" || ack === 1 || ack === "1") {
                thenFn();
                return;
            }
        } catch { /* ignore */ }
    ...
    function openAdminToolsWithDisclaimer() {
        if (!INCLUDE_ADMIN_TOOLS) return;
        if (INCLUDE_FULL_DEVELOPER_TOOLS && isDevPreviewAdminMainMenuEnabled()) {
            runAdminSurfaceWithDisclaimer(() => openAdminToolsMenu(), { forcePublicDisclaimer: true });
            return;
        }
        runAdminSurfaceWithDisclaimer(() => openAdminToolsMenu());
    }
    ...
    function openDeveloperToolsWithDisclaimer() {
        if (!INCLUDE_FULL_DEVELOPER_TOOLS) {
            if (INCLUDE_ADMIN_TOOLS) openAdminToolsWithDisclaimer();
            return;
        }
        try {
            const ack = getPlayerProperty(player, DEVELOPER_TOOLS_DISCLAIMER_PROP);
            if (ack === true || ack === "true" || ack === 1 || ack === "1") {
                openDeveloperTools();
                return;
            }
    ...
    function openHeavyPerfPresetsMenu() {
        const stormM = getStormWorkManualOrZero();
        const minM = getMiningWorkManualOrZero();
        ...
        form.show(player).then((res) => {
            ...
            player.playSound("mb.codex_turn_page", { pitch: 1.1, volume: 0.7 * getPlayerSoundVolume(player) });
            if (res.selection === 0) return openStormWorkPresetMenu();
            if (res.selection === 1) return openMiningWorkPresetMenu();
            if (res.selection === 2) {
                setSpatialSpawnTuningEnabled(!spatial);
                try { saveAllProperties(); } catch { /* ignore */ }
                player.sendMessage(CHAT_INFO + "Spatial spawn groups: " + (!spatial ? "ON (cluster-aware budgets)." : "OFF (full spread cost)."));
                return openHeavyPerfPresetsMenu();
            }
            return journalPowerToolsBack();
        }).catch(() => journalPowerToolsBack());
    ...
    function openStormWorkPresetMenu() {
        const form = new ActionFormData()
            .title("§bStorm work cadence")
            .body("§7Named presets §8(like spawn intensity). §7Higher = less storm CPU per tick.\n§8Auto §7clears manual override.");
        ...
        form.show(player).then((res) => {
            ...
            player.playSound("mb.codex_turn_page", { pitch: 1.1, volume: 0.7 * getPlayerSoundVolume(player) });
            const row = STORM_WORK_PRESET_ROWS[res.selection];
    ...
    function openMiningWorkPresetMenu() {
        const form = new ActionFormData()
            .title("§bMining AI cadence")
            .body("§7Named presets §8(like spawn intensity). §7Higher = mining batches run less often.\n§8Auto §7clears manual override.");
        ...
        form.show(player).then((res) => {
            if (!res || res.canceled || res.selection === backIdx) return openHeavyPerfPresetsMenu();
            player.playSound("mb.codex_turn_page", { pitch: 1.1, volume: 0.7 * getPlayerSoundVolume(player) });
            const row = MINING_WORK_PRESET_ROWS[res.selection];
    ...
    /**
     * One tap: spawn + scan + storm work mult + mining mult (or clear storm/mining manuals).
     * @param {{ spawnKey: string, scanKey: string, stormWorkMult?: number, miningWorkMult?: number, clearHeavyManuals?: boolean }} c
     */
    function applyWorldPerfCombo(c) {
        if (!c) return;
        applySpawnAndScanCombo(c.spawnKey, c.scanKey);
        if (c.clearHeavyManuals) {
            setStormWorkMultiplierManual(undefined);
            setMiningWorkMultiplierManual(undefined);
        } else {
            setStormWorkMultiplierManual(c.stormWorkMult);
            setMiningWorkMultiplierManual(c.miningWorkMult);
        }
        try { saveAllProperties(); } catch { /* ignore */ }
    }
    ...
    function openWorldPerfComboMenu() {
        ...
        form.show(player).then((res) => {
            ...
            player.playSound("mb.codex_turn_page", { pitch: 1.1, volume: 0.7 * getPlayerSoundVolume(player) });
            const combo = WORLD_PERF_COMBO_PRESETS[res.selection];
            if (combo) {
...
§7Dust storms §8(world): §f${dustOn ? "§aON" : "§cOFF"}
§7Multiple storms: §f${multiOn ? "§aON" : "§cOFF"}
...
§7Intensity: §f${ctrl.intensity != null ? ctrl.intensity.toFixed(2) : "auto"} §7| Max storms: §f${ctrl.maxConcurrentStorms} §7| Secondary: §f${(ctrl.secondaryStormChance * 100).toFixed(0)}%`;
        ...
        const form = new ActionFormData()
            .title("§6Storm")
            .body(body);
        form.button(dustOn ? "§cDust storms OFF §8(world)" : "§aDust storms ON §8(world)");
        form.button(multiOn ? "§cMultiple storms OFF" : "§aMultiple storms ON");
        ...
        form.show(player).then((res) => {
            ...
            try {
                switch (res.selection) {
                    case 0:
                        setDustStormsEnabled(!dustOn);
                        try { saveAllProperties(); } catch { /* ignore */ }
                        player.sendMessage(CHAT_DEV + "[MBI] " + CHAT_INFO + `Dust storms (world): ${!dustOn ? "ON" : "OFF"}`);
                        openStormHubMenu();
                        break;
                    case 1:
                        setMultiStormEnabled(!multiOn);
                        try { saveAllProperties(); } catch { /* ignore */ }
                        player.sendMessage(CHAT_DEV + "[MBI] " + CHAT_INFO + `Multiple dust storms: ${!multiOn ? "ON" : "OFF"}`);
                        openStormHubMenu();
                        break;
                    case 2: openSummonStormTargetMenu("minor"); break;
                    case 3: openSummonStormTargetMenu("major"); break;
                    case 4:
                        if (state.active) {
                            endStorm(true);
                        ...
                        openStormHubMenu();
                        break;
                    case 5: openStormListMenu(); break;
                    case 6: openStormOverrideMenu(); break;
                    case 7: openStormControlSettingsMenu(); break;
                    case 8: openSnowStormDebugMenu(getDebugSettings(player), true); break;
...
§7Particles: ${stormSettings.particles ? "§aON" : "§cOFF"}`;
    ...
    } else if (entry === "admin" && INCLUDE_ADMIN_TOOLS) {
        try { openAdminToolsWithDisclaimer(); } catch { openMain(); }
    } else if (entry === "dev" && INCLUDE_FULL_DEVELOPER_TOOLS) {
        try { openDeveloperToolsWithDisclaimer(); } catch { openMain(); }
    } else if (entry === "debug" && INCLUDE_FULL_DEVELOPER_TOOLS) {
        try { openDebugMenu(true); } catch { openMain(); }
    } else {
        try { openMain(); } catch { /* ignore */ }
    }
}
...
function showSettingsChooserBasic(player) {
    ...
    form.show(player).then((res) => {
        ...
        if (res.selection === 1) {
            return openJournalLagComfortWizard(player, () => showSettingsChooserBasic(player));
        }
        const volumeMultiplier = getPlayerSoundVolume(player);
        player.playSound("mb.codex_turn_page", { pitch: 1.0, volume: 0.8 * volumeMultiplier });
        showBasicJournalUI(player);
